# -*- coding: utf-8 -*-
"""
Created on Tue Sep 26 14:36:14 2017
...
def lr_train_bgd(feature,label,maxCycle,alpha):
    ...
    while i<=maxCycle:   #  在最大迭代次数的范围内
        i+=1   # 当前的迭代次数
        h=sig(feature*w)   # 计算Sigmoid值  feature*w矩阵相乘,其维数：m*r * r*n = m*n
        err=label-h
        if i%100 ==0:
            print "\t--------iter="+str(i)+\
...
def save_model(file_name,w):
    '''
    保存模型参数w
    input:file_name(string):保存模型的文件名
          w(mat):LR模型的权重
    '''
    m=np.shape(w)[0]
